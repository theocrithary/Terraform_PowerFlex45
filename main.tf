...
resource "null_resource" "powerflex_node_2_bootstrap" {
  ...
  provisioner "remote-exec" {
    inline = [
      "apt update -y",
      "apt install unzip sshpass numactl libaio1 wget libapr1 libaprutil1 bash-completion binutils openjdk-11-jdk-headless smartmontools sg3-utils hdparm pciutils sysstat jq openssl libaio1 linux-image-extra-virtual libnuma1 -y",
      "sed -i '/- ${var.powerflex_node_2_ip}/a${indent(8,- ${var.powerflex_node_2_ip_rep}/${var.subnet_netmask})}' /etc/netplan/99-netcfg-vmware.yaml",
      "netplan apply"
    ]
  }
}
...
resource "null_resource" "powerflex_node_3_bootstrap" {
  ...
  provisioner "remote-exec" {
    inline = [
      "apt update -y",
      "apt install unzip sshpass numactl libaio1 wget libapr1 libaprutil1 bash-completion binutils openjdk-11-jdk-headless smartmontools sg3-utils hdparm pciutils sysstat jq openssl libaio1 linux-image-extra-virtual libnuma1 -y",
      "sed -i '/- ${var.powerflex_node_3_ip}/a${indent(8,- ${var.powerflex_node_3_ip_rep}/${var.subnet_netmask})}' /etc/netplan/99-netcfg-vmware.yaml",
      "netplan apply"
    ]
  }
}
...
resource "null_resource" "powerflex_node_4_bootstrap" {
  ...
  provisioner "remote-exec" {
    inline = [
      "apt update -y",
      "apt install unzip sshpass numactl libaio1 wget libapr1 libaprutil1 bash-completion binutils openjdk-11-jdk-headless smartmontools sg3-utils hdparm pciutils sysstat jq openssl libaio1 linux-image-extra-virtual libnuma1 -y",
      "sed -i '/- ${var.powerflex_node_4_ip}/a${indent(8,- ${var.powerflex_node_4_ip_rep}/${var.subnet_netmask})}' /etc/netplan/99-netcfg-vmware.yaml",
      "netplan apply"
    ]
  }
}