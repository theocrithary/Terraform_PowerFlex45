...
#######################################################################
## Stage 4: Run the installer scripts for PFMP configuration
#######################################################################

# Wait for all storage nodes to be available and complete the bootstrap process

resource "time_sleep" "wait_for_bootstrap" {
  create_duration = "300s"
  depends_on = [ null_resource.powerflex_node_1_bootstrap, null_resource.powerflex_node_2_bootstrap, null_resource.powerflex_node_3_bootstrap, null_resource.powerflex_node_4_bootstrap ]
}

## Login as root to setup and install PFMP
resource "null_resource" "pfmp-installer-scripts" {
  depends_on = [time_sleep.wait_for_rootpasswordchange3]
  connection {
      type     = "ssh"
      user     = "root"
